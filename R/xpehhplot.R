xpehhplot <- function (xpehh.data, file.name, file.type = 'png', plot.pval = TRUE, ylim.scan = 2, pch = 16, cex = 0.3, 
  ...
  if (!(grepl("XP-EHH", colnames(xpehh.data)[3]))) {
    stop("Unrecognized column name for XP-EHH: the xpehh.data is not a matrix generated by the ies2xpehh() function...")
  }
  ...
  if (nbr_chrm > 1) {
    ...
    plot(pos, xpehh.data[, 3], pch = pch, cex = cex, las = 1, col = col_chr, xaxt = "n", xlab = "Chromosome", 
         ylab = expression(italic(XP-EHH)), cex.lab = cex.lab, main = main, cex.main = cex.main, cex.axis = cex.axis)
    ...
    plot(pos/scale, xpehh.data[, 3], pch = pch, col = col_chr, 
         xlab = paste("Position", unit), ylab = expression(italic(XP-EHH)), 
